...
if (isset($_POST['submit'])) {
  ...
  if (empty($cel)) {
    ...
    $sql2 = "SELECT * FROM students WHERE cel_id ='$cel'";
    $resultado2 = mysqli_query($link, $sql2);
    $filas2 = mysqli_fetch_array($resultado2);

    if ($filas2 > 1) {
      $cel_error = 'Ya existe este numero vinculado a otro codigo';
    } else {
      ...
      $updatecel = "UPDATE `students` SET `cel_id`='$cel',`activo`='1' WHERE `id_student`='$id'";
      $resultadou = mysqli_query($link, $updatecel);

      $rows = mysqli_affected_rows($link);
      if($rows>0){
        echo "<script language='JavaScript'>
              alert('Se vinculo exitosamente el numero a este codigo');
              location.assign('./mainUser.php');
